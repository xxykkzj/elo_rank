...
# Create tibbles to store validation statistics for both training and testing sets
```{r}
...
# Combine the validation statistics for both sets
```{r}
...
# Create a tibble to store validation statistics for the logistic regression model on the training set
```{r}
validation_stats_train <- tibble(
  model = "logistic",
...
# Predict probabilities of winning for the testing set
```{r}
...
# Create a tibble to store validation statistics for the logistic regression model on the testing set
```{r}
validation_stats_test <- tibble(
  model = "logistic",
...
# Combine the validation statistics for both training and testing sets
```{r}
validation_stats <- bind_rows(validation_stats_train, validation_stats_test)
```
...
# Define the Elo update function for the K-factor model
```{r}
...
# Define the Elo update function for the FiveThirtyEight model
```{r}
...
# Iterate through matches and update Elo scores for training set
```{r}
...
# Iterate through matches and update Elo scores for testing set
```{r}
...
# Get a list of all players from matches_df
```{r}
all_players <- unique(c(matches_df$winner_id, matches_df$loser_id))
```


# Filter elo_scores to only contain information about players in all_players
```{r}
elo_scores <- elo_scores[elo_scores$player_id %in% all_players, ]
head(elo_scores)
```

# Filter matches_train_df and matches_test_df to only contain information about players in elo_scores
```{r}
...
# Recalculate elo_predictions_train and elo_predictions_test
```{r}
...
# Recalculate elo_probs_train and elo_probs_test
```{r}
...
# Recalculate accuracy, log-loss, and calibration for the training set and testing set
```{r}
...
```{r}
#save dataset
#saveRDS(filtered_df, file = "C:\\Users\\biand\\Downloads\\tennis_atp\\project\\filtered_ds.rds")
#write.csv(filtered_df, file = "C:\\Users\\biand\\Downloads\\tennis_atp\\project\\filtered_ds.csv")
```
